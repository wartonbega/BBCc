...
pub const tokenReader = struct {
    ...
    pub fn new(toks: ArrayList(parser.Token), uri: []const u8) tokenReader {
        return .{ .pos = 0, .tokens = toks, .uri = uri };
    }
...
pub fn lexeValue7(reader: *tokenReader, allocator: Allocator) (std.mem.Allocator.Error || lexerError || std.fmt.ParseIntError)!*ast.Value {
    ...
    switch ((try reader.peek()).type) {
        ...
        TokenType.AT => { // Struct usage
            ...
            stc_init.reference = at_op.location;
        ...
        TokenType.PRINT, TokenType.PRINTLN => |tok| {
            ...
            ret.* = ast.Value{ .Print = .{
                ...
                .ln = tok == .PRINTLN,
                .reference = print_token.location,
            } };
            return ret;
        },
        else => {
...
pub fn lexeValue0(reader: *tokenReader, allocator: Allocator) (lexerError || std.mem.Allocator.Error || std.fmt.ParseIntError)!*ast.Value {
    ...
    if (reader.canPeek() and (try reader.peek()).type == TokenType.EQUAL) {
        ...
        assignement.* = ast.Assignement{
            .lhs = lhs,
            .rhs = rhs,
            .reference = assignation_op.location,
        };
...
pub fn lexeScope(reader: *tokenReader, allocator: Allocator) !*ast.Scope {
    ...
    const ret = try ast.Scope.init(values, ocur.location, allocator);
    return ret;
}
...
pub fn lexeFuncdef(reader: *tokenReader, allocator: Allocator) !*ast.funcDef {
    ...
    if (reader.canPeek() and ((try reader.peek()).type == .PRINT or (try reader.peek()).type == .PRINTLN)) {
        errors.bbcErrorExit("Unauthorized name '{s}' for a function, try '_{s}' if you really want to use that name", .{ (try reader.peek()).value, (try reader.peek()).value }, fn_kwd.location);
    }
...
pub fn lexeStructDef(reader: *tokenReader, allocator: Allocator) !*ast.structDef {
    ...
    ret.reference = struct_keyword.location;
...
pub fn lexeProgram(tokens: ArrayList(parser.Token), allocator: Allocator, uri: []const u8) !*ast.Program {
    ...
    while (reader.canPeek()) {
        ...
        switch (token.type) {
            ...
            //     const import_token = reader.consume(.IMPORT);
            //     const file_name = reader.consume(.STRINGLIT).value;
            //     // File name can be the name of the file (relative path to the file)
            //     // or the name of a file in the include dir
            // },
            else => {
                errors.bbcErrorExit("Unexpected token {}", .{token.type}, token.location);
            },
        }
    }
    //ret.print();
