...
pub const Token = struct { type: TokenType, value: []const u8, pos: []const u8 };
...
const Reader = struct {
    ...
    pub fn new(allocator: *std.heap.ArenaAllocator, filename: []const u8) !Reader {
        ...
        return Reader{ .content = contents, .pos = 0, .filename = filename };
    }
    ...
    // Returns the current position as a formated string :
    //      filename:line:column
    pub fn getCurrentPosition(self: *Reader, allocator: std.mem.Allocator) ![]const u8 {
        var line = @as(u32, 0);
        var col = @as(u32, 0);
        for (0..self.pos) |i| {
            if (self.content[i] == '\n') {
...
fn parseIntegerLiteral(reader: *Reader, allocator: std.mem.Allocator) !Token {
    ...
    return .{ .type = TokenType.INTLIT, .value = value, .pos = try reader.getCurrentPosition(allocator) };
}
...
fn parseIdentifier(reader: *Reader, allocator: std.mem.Allocator) !Token {
    ...
    const value = buffer[0..pos];
    return .{ .type = getIdentType(value), .value = value, .pos = try reader.getCurrentPosition(allocator) };
}

fn parseChar(reader: *Reader, allocator: std.mem.Allocator) !Token {
    _ = reader.consume(1);
    const char = reader.consume(1);
    const closing_quote = reader.consume(1)[0];
    if (closing_quote != '\'')
        return parser_error.ExpectedToken;
    return .{ .type = TokenType.CHARLIT, .value = char, .pos = try reader.getCurrentPosition(allocator) };
}

fn parseString(reader: *Reader, allocator: std.mem.Allocator) !Token {
    _ = reader.consume(1);
    var i: u32 = @intCast(1);
...
fn parseOperator(reader: *Reader, allocator: std.mem.Allocator) !Token {
    ...
    // if (reader.canPeek(1)) no need to check : there is an operator recognized
        ...
        const ret = switch (char) {
            ...
            '*' => Token{ .type = TokenType.TIMES, .value = "*", .pos = try reader.getCurrentPosition(allocator) },
...
pub fn parse(filename: []const u8, arena: *std.heap.ArenaAllocator) !std.ArrayList(Token) {
    ...
    while (true) {
        ...
        if (reader.canPeek(2) and std.mem.eql(u8, reader.peek(2), "//")) { // line comment
            ...
            try tokens.append(try parseChar(&reader, allocator));
        } else if (char == '"') {
            try tokens.append(try parseString(&reader, allocator));
        } else if (char == ',') {
            try tokens.append(.{ .type = TokenType.COMMA, .value = reader.consume(1), .pos = try reader.getCurrentPosition(allocator) });
        } else if (char == '{') {
            try tokens.append(.{ .type = TokenType.O_CUR, .value = reader.consume(1), .pos = try reader.getCurrentPosition(allocator) });
        } else if (char == '}') {
            try tokens.append(.{ .type = TokenType.C_CUR, .value = reader.consume(1), .pos = try reader.getCurrentPosition(allocator) });
        } else if (char == '(') {
            try tokens.append(.{ .type = TokenType.O_PAR, .value = reader.consume(1), .pos = try reader.getCurrentPosition(allocator) });
        } else if (char == ')') {
            try tokens.append(.{ .type = TokenType.C_PAR, .value = reader.consume(1), .pos = try reader.getCurrentPosition(allocator) });
        } else if (char == '[') {
            try tokens.append(.{ .type = TokenType.O_BRA, .value = reader.consume(1), .pos = try reader.getCurrentPosition(allocator) });
        } else if (char == ')') {
            try tokens.append(.{ .type = TokenType.C_BRA, .value = reader.consume(1), .pos = try reader.getCurrentPosition(allocator) });
